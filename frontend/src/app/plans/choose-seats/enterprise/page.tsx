...
export default function ChooseEnterpriseSeats() {
  const [seatCount, setSeatCount] = useState(1);
  const [plan, setPlan] = useState<SeatPlan | null>(null);
  const { seatPlans, loading } = usePlans();

  useEffect(() => {
    // Load Enterprise seat plan from API/context
    const enterprisePlan = seatPlans.find((p: any) => p.id === 'enterprise-seat');
    if (enterprisePlan) setPlan(enterprisePlan);
  }, [seatPlans]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 flex-grow grid grid-cols-1 sm:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-gray-500 mb-1">Enterprise Seat Plan</p>
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600 mb-4">{plan?.description || 'Full feature access for power users and administrators'}</p>
        ...
        <div className="bg-gray-100 p-8 rounded-lg shadow mt-8 sm:mt-0 flex flex-col justify-between flex-grow">
          ...
          <div className="text-sm space-y-2">
            <div className="grid grid-cols-3 gap-x-2">
              ...
              <span>{plan?.name || 'Enterprise Seat'}</span>
            </div>
            <div className="grid grid-cols-3 gap-x-2">
